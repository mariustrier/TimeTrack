...
export async function POST(req: Request) {
  try {
    ...
    const { userId, weekStart, reason } = result.data;
    ...
    if (entries.length === 0) {
      return NextResponse.json({ error: "No submitted entries found for this week" }, { status: 400 });
    }
    ...
    await db.auditLog.create({
      data: {
        ...
        toStatus: "draft",
        actorId: user.id,
        metadata: JSON.stringify({ weekStart, userId, reason, entryCount: entries.length }),
      },
    });
