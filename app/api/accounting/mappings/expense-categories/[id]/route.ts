...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const mapping = await db.expenseCategoryMapping.findFirst({
      where: { id: params.id, companyId: user.companyId },
    });
    if (!mapping) return NextResponse.json({ error: "Not found" }, { status: 404 });

    await db.expenseCategoryMapping.delete({ where: { id: mapping.id } });
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("[EXPENSE_CATEGORY_MAPPING_DELETE]", error);
