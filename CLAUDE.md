...
- `components/projects/` - ProjectsList, ProjectTimeline (self-contained ~3200-line inline-styled Gantt), PhaseProgress, TimelineExportPopover (Excel/PDF export with filter popover, fully i18n'd via `timelineExport` namespace), TimelinePdfDocument (@react-pdf/renderer A3 Gantt PDF)
...
- `components/resource-planner/` - ResourceGrid, AllocationDialog, AllocationBlock, ViewControls, PlannerControls, PlannerGrid, PlannerRow, PlannerCell, BulkActionToolbar, CapacitySummary, PlannerSummary, TotalRow, VacationBlock, CapacityColumn
- `components/team/` - TeamList, ResourcePlanner
- `components/vacations/` - VacationCalendar, VacationPlanner
- `components/layout/` - Sidebar (role-based nav with badge counts, support mode banner), PageHeader (standardized inline-styled page header matching Analytics aesthetic — DM Sans, indigo active pills, used by all dashboard pages)
...
- `components/ui/` - shadcn/ui primitives, guided-tour, page-guide, info-tooltip, theme/locale toggles, FetchError (reusable API error state with retry button), UserAvatar (initials fallback with deterministic colors, supports uploaded avatarUrl)
...
- `lib/image-resize.ts` - Client-side image resize/compress (canvas-based, used by avatar upload)
...
- 5 stat cards: Target, Billable Hours, Vacation (merged — shows remaining as value, "{used} used · {remaining} remaining" subtitle), Total Hours, Flex Balance (Flexsaldo). All numeric values use `.toFixed(1)` for clean display. Collapsible via "Show more"/"Show less" toggle.
...
### Team
- **Team tab**: Member list with roles, bill/cost rates, weekly targets, extra vacation days, employment type (employee/freelancer), hourly toggle, category assignment, profile picture upload
- **Profile pictures (avatars)**: Admin/managers can upload profile photos per employee (2MB max, PNG/JPG/WebP). Stored via Vercel Blob (`User.avatarUrl`). `UserAvatar` component renders uploaded photo or falls back to colored initials. Used in team table, approvals list, audit log, and timeline. API: `POST/DELETE /api/team/[id]/avatar`. Client-side resize via `lib/image-resize.ts`.
- **Employee categories (Roller)**: Assignable via "Kategori" dropdown in team edit/invite modal. Categories are company-scoped roles managed in Admin Overview (see below). Shown as badge in team table. Stored as `User.roleId` → `Role` relation. "Ingen kategori" option for unassigned.
...
- **Resource Planner tab**: Week/2-week/month grid view with **period span slider** (Week/2-week: 1-4wk, Month: 2-6mo)
  ...
  - Dark-mode-aware heatmap cells (`heatColor()` in PlannerCell returns Tailwind classes with `dark:` variants for all 5 utilization tiers)
...
- **API error handling convention**: All data-fetching components use `<FetchError>` from `components/ui/fetch-error` when API calls fail. Pattern: `const [error, setError] = useState<string | null>(null)` → set in `catch` / `!res.ok` → clear on success → render `<FetchError message={error} onRetry={fetchFn} />` after loading check. Translation keys in `common` namespace: `fetchErrorTitle`, `fetchErrorDescription`, `tryAgain`.
